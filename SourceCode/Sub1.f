...
c
      ...
      real term1, term2, term3, ur, x, z
      real cthr, dFzinc, stgp, ctinc, cdl, r2d, dz
...
c
c       Increment in tangential force due to radial induction
        dFzinc = dens * gam * dr * ur * tan(kappa)
        stgp   = 0.5 * dens * w2
        ctinc  = dFzinc / (stgp * B * chord * dr)
        ct     = ct + ctinc
...
c
cDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
c----  Calculate radial induction (ur) using Rational Algebraic Expansion
        term1 = vwind / 2.24
        term2 = cthr / (4.0 * pi)
        x     = r / rtip
        
c----  Rational substitution 'z'
        z     = (2.0 * x) / (0.04**2 + x**2 + 1.0)
        
c----  3-term algebraic expansion
        term3 = 2.0*z + (2.0/3.0)*(z**3) + (2.0/5.0)*(z**5)
        ur    = term1 * term2 * term3

        AA     = B*chord*cn/(8.*TL*pi*r*sin(phi)*sin(phi))
c
c--
c--------(Start Code - added & updated by Bhima on 09.06.2026)
c----Calculate new axial induction (a) using selected numerical solver
c       -------------------------------------------------------------------
        call Calc_Induction_Convergence(iiters, AA, ac, chord, r, phi, 
     +               cn, TL, twist, aset, da, aold, anew, err, iters)
c       -------------------------------------------------------------------
...
c
	abem(i) = anew
        apbem(i)= apnew
c
c       Include radial velocity component and drag in circulation ----(Start Code - updated by Bhima on 26.05.2026)
        w2  = w2 + ur*ur
        cdl = sqrt(clo**2 + cdo**2)
        gam = .5 * cdl * sqrt(w2) * chord
...
c       Convert kappa to degrees for the output file
     ...
     +              iter,err,th,nameprout,iters,dFx,dFz,r2d*kappa,
     +              cthr,ur
c
...
c--------(Start Code - added & updated by Bhima on 09.06.2026)
     ...
     +           phi, cn, TL, twist, aset, da, aold, anew, err, iters)
c-----------------------------------------------------------------------
      use mem
