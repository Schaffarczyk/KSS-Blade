...
c
      ...
      real cT_loc, term1, term2, term3, ur_madsen, top, bot
...
c
c     --- Calculate Kappa (Local Dihedral Angle) ----(Start Code - updated by Bhima on 05.05.2026)
      if (r .le. 2.8667) then
         kappa = 0.0
      else
         kappa = 0.09955
      endif
c ----(End Code - updated by Bhima on 05.05.2026)
...
c----------------------TEST IMPLEMENTATION: Mod Eqn 6-------(Start Code - updated by Bhima on 19.05.2026)
c---------------------   
c------------Calculate local thrust coefficient (cT_loc) & Calculate Modified Madsen Radial Induction (ur_madsen)
      cT_loc = (b * chord * w2 * cn) / (vwind**2 * 2.0 * pi * r)

      term1 = vwind / 2.24
      term2 = cT_loc / (4.0 * pi)
      
      top = 0.04**2 + (r/rtip + 1.0)**2
      bot = 0.04**2 + (r/rtip - 1.0)**2
      term3 = log(top / bot)
      
      ur_madsen = term1 * term2 * term3
c    ------------------------------------------(End Code - updated by Bhima on 19.05.2026)
        write(*,100)r,anew,apnew,TL,sqrt(w2),chord,twist,
     +              phi*57.3,aoab,clo,cdo,clo/cdo,cn,ct,dTa,dFta,gam,
     +               iter,err,th,nameprout,iters,dFx,dFz,kappa,cT_loc,ur_madsen
c
